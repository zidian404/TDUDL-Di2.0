...
class FixedSobel(nn.Module):
    def __init__(self):
        super(FixedSobel, self).__init__()
...
class TextureGuidance(nn.Module):
    def __init__(self, x_channels=16, out_channels=16):
        super(TextureGuidance, self).__init__()
        self.sobel = FixedSobel()
        self.hf = HighFreqExtractor(channels=1)
        self.fuse = nn.Sequential(
            nn.Conv2d(x_channels + 3, out_channels, 3, padding=1, bias=True),
            nn.GELU(),
    ...
    def forward(self, x):
        ...
        edge_mag, edge_ori = self.sobel(g)
        high = self.hf(g)
        ori_cos = torch.cos(edge_ori)
        feat = torch.cat([x, edge_mag, high, ori_cos], dim=1)
        return self.fuse(feat)


class BodyNet(nn.Module):
    def __init__(self, unet, S, S_T, dict_adapter: DictAdapter, tex_guidance: TextureGuidance):
        super(BodyNet, self).__init__()
        self.unet = unet
        self.S = S
        self.S_T = S_T
        self.dict_adapter = dict_adapter
        self.tex_guidance = tex_guidance
    ...
    def forward(self, X_in, Y, Z, beta, alpha, rho, gamma, samfeats, enc, dec):
        ...
        tex_feat = self.tex_guidance(X_out)
        Z_in = torch.cat([tex_feat, rho_], dim=1)
        Z, samfeats, enc_, dec_ = self.unet(Z_in, samfeats, enc, dec, stage_inter=True)

        beta = gamma[0] * beta + gamma[1] * X_out - gamma[2] * Z
        return X_out, Z, beta, samfeats, enc_, dec_


class denoise_Net_admm_restormer_tex(nn.Module):
    def __init__(self, opt):
        super(denoise_Net_admm_restormer_tex, self).__init__()
        ...
        self.tex_guidance = TextureGuidance(Cx, Cx)
        self.body = BodyNet(self.unet, self.S, self.S_T, self.dict_adapter, self.tex_guidance)
    ...
    def forward(self, input: Tensor, sigma: Tensor):
        ...
        for k in range(self.stage):
            ...
            if k == 0:
                ...
                tex_feat = self.tex_guidance(X)
                Z, samfeats, enc, dec = self.unet(torch.cat([tex_feat, rho_map], dim=1), samfeats, enc, dec, stage_inter=True)

                beta = gamma[1] * X - gamma[2] * Z

                w_out = self.dict_adapter(X)
                output = self.S(X) + (self.S(X * w_out) - self.S(X))
                preds.append(output)
            else:
        ...
        output = self.S(X_out) + (self.S(X_out * w_out) - self.S(X_out))
        preds.append(output)
        return output, preds