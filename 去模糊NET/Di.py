...
class denoise_Net_admm_restormer(nn.Module):
    ...
    def forward(self, input, blur_level: Tensor = None):
        device = input.device
        b = input.size(0)

        # 让 blur_level 可以是 None
        ...
        # 2. 初始化
        X = self.S_T(self.headnet(input, blur_level))
        ...
        for k in range(self.stage):
            hypas = self.hypa_list[k](blur_level)
